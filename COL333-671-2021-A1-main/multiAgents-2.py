...
def betterEvaluationFunction(currentGameState):
    ...
    final_eval_score = currentGameState.getScore()*0.1
    ...
    if currentGameState.isWin():
        final_eval_score+= 1e10
        return final_eval_score
    if currentGameState.isLose():
        final_eval_score+= -1e10
        return final_eval_score
    final_eval_score += -100*(len(capsules))
    final_eval_score += -100*(len(foods))
    print("Position:", pacman_position)
    print("Initial score:", currentGameState.getScore())
    food_scores_list = []
    minsofar=1e11
    if len(foods) ==0:
        return 1e10
    for food1 in foods:
        minsofar = min(minsofar, manhattanDistance(pacman_position, food1))
    
    final_eval_score -= 10*minsofar
    ...
    for ghost in ghosts:
        ...
        if ghost.scaredTimer:
            ...
            if ghost_distance < 2:
                ghost_score_list.append(0.4*ghost_distance)
            elif ghost_distance < 6:
                ghost_score_list.append(0.2*ghost_distance)
            else:
                ghost_score_list.append(0.05*ghost_distance)
    final_eval_score += sum(ghost_score_list)
    print("Ghost score:", sum(ghost_score_list))
    for capsule in capsules:
        capsule_distance = manhattanDistance(pacman_position, capsule)
        if capsule_distance < 2:
            final_eval_score += -30*capsule_distance
        else:
            final_eval_score += -4*capsule_distance

    print("Final score:", final_eval_score)
    return final_eval_score
    # util.raiseNotDefined()
